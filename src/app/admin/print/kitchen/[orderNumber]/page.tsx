...
function getRootParentName(categoryId: string, catById: Map<string, CategoryRow>): string {
  ...
  if (!cat) return "";
  if (!cat.parent_id) return normalize(cat.name);
...
export default async function PrintKitchenPage({
  ...
  const kitchenItems = (items ?? []).filter((it) => {
    ...
    return getRootParentName(catId, catById) === "makanan";
  });
    ...
    <main className="mx-auto p-4 print:p-0" style={{ width: "80mm" }}>
      ...
      <div className="space-y-3 text-sm">
        <div className="text-center border-b-2 border-solid pb-3">
          ...
          <div className="text-xs font-semibold uppercase tracking-widest opacity-70">Struk Dapur</div>
        </div>
        ...
        {kitchenItems.length === 0 ? (
          <div className="text-xs opacity-60 text-center py-3">Tidak ada item makanan.</div>
        ) : (
