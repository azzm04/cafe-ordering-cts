...
export default function MenuPage() {
  ...
  // ✅ Filter berdasarkan parent category name
  const filtered = useMemo(() => {
    const want = tab.toLowerCase();
    return items.filter((it) => {
      // Ambil parent category name
      const parentName = it.categories?.parent_category?.name ?? it.categories?.name ?? "";
      return parentName.toLowerCase() === want;
    });
  }, [items, tab]);
