...
const steps = [
  { status: "received" as const, label: "Diterima", icon: "📝", description: "Order masuk" },
  { status: "preparing" as const, label: "Sedang Dibuat", icon: "👨‍🍳", description: "Diproses" },
  { status: "served" as const, label: "Disajikan", icon: "🍽️", description: "Siap diantar" },
  { status: "completed" as const, label: "Selesai", icon: "✅", description: "Terima kasih" },
] as const;
    ...
    <div className="w-full py-6 px-2">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.status} className="flex flex-col items-center flex-1">
                <div
                  className={[
                    "relative w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center text-xl sm:text-2xl",
      ...
      <div className="mt-6 text-center">
        <div className="inline-block px-4 py-2 bg-primary/10 rounded-full">
          <p className="text-sm font-semibold text-primary">{steps[currentIndex]?.label}</p>
        </div>
      </div>
    </div>
  );
